Feature: Mark To-Do as Completed
  ...
  Scenario: Mark a to-do item as completed
    Given a to-do item exists with ID "123" and is not marked as completed
    When I send a PUT request to update the status to "true" for the to-do item with ID "123"
    Then the system should mark the to-do item with ID "123" as completed
  ...
  Scenario: Mark a completed to-do item as not completed
    Given a to-do item exists with ID "123" and is marked as completed
    When I send a PUT request to update the status to "false" for the to-do item with ID "123"
    Then the system should mark the to-do item with ID "123" as not completed
